...
{
	return ImGui::Begin(unicodeToUTF8(name).c_str(), open, flags == nullptr ? 0 : *flags);
}
...
{
	return ImGui::BeginChild(unicodeToUTF8(str_id).c_str(), getImVec2(size), border != nullptr ? *border : FALSE, flags != nullptr ? *flags : 0);
}

HL_PRIM bool HL_NAME(begin_child2)(int id, vdynamic* size, bool* border, ImGuiWindowFlags* flags)
{
	return ImGui::BeginChild(ImGuiID(id), getImVec2(size), border != nullptr ? *border : FALSE, flags != nullptr ? *flags : 0);
}
